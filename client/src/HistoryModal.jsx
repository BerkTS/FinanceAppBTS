...
function Heading({ children, variant }) {
  ...
  return (
    <h3
      className={`mb-2 border-b pb-1.5 text-2xl font-bold tracking-tight ${styles[variant] || styles.muted}`}
    >
      {children}
    </h3>
  );
...
function RenderStructuredBulk({ data }) {
  ...
  if (data?.loading) return <p className="text-lg text-muted">Was loading…</p>;
  if (data?.err) return <p className="text-lg text-amber-200/90">{data.err}</p>;
  if (!d) return <p className="text-lg text-muted">No data</p>;
  if (d.needsSchwab) return <p className="text-lg text-muted">Schwab not connected that day.</p>;
  const views = d.views || [];
  if (views.length === 0) return <p className="text-lg text-muted">No views captured.</p>;
  return (
    <div className="space-y-3">
      {d.disclaimer && (
        <p className="rounded border border-amber-500/25 bg-amber-500/5 px-2 py-1.5 text-lg leading-snug text-amber-100/90">
          {d.disclaimer}
        </p>
      )}
      {views.map((row) => (
        <div key={row.symbol} className="rounded-lg border border-surface-border bg-black/20 p-3 text-xl">
          <p className="font-mono font-semibold text-emerald-200">{row.symbol}</p>
          {row.ruleTargets && (
            <p className="mt-1 font-mono text-lg text-slate-400">
          ...
          {row.claude?.thesis && (
            <p className="mt-2 text-xl text-violet-200/90">
              <span className="font-semibold text-violet-300">Claude:</span> {row.claude.thesis}
            </p>
          )}
          {row.openai?.thesis && (
            <p className="mt-2 text-xl text-sky-200/90">
...
function RenderTradeSuggest({ data }) {
  ...
  if (data?.loading) return <p className="text-lg text-muted">Was loading…</p>;
  if (data?.err) return <p className="text-lg text-amber-200/90">{data.err}</p>;
  if (!d?.suggestions?.length) return <p className="text-lg text-muted">No suggestions captured.</p>;
  return (
    <ul className="space-y-3 text-xl">
      {d.suggestions.map((s) => (
        <li key={s.symbol} className="rounded-lg border border-surface-border bg-black/15 p-3">
          ...
          {s.claude?.thesis && <p className="mt-2 text-xl text-slate-300">Claude: {s.claude.thesis}</p>}
          {s.chatgpt?.thesis && <p className="mt-2 text-xl text-slate-300">ChatGPT: {s.chatgpt.thesis}</p>}
        </li>
      ))}
    </ul>
  );
...
function RenderBriefing({ data }) {
  ...
  if (data?.loading) return <p className="text-lg text-muted">Was loading…</p>;
  if (data?.err) return <p className="text-lg text-amber-200/90">{data.err}</p>;
  if (!d) return <p className="text-lg text-muted">No briefing</p>;
  if (d.skipped && d.reason === "no_api_key")
    return <p className="text-lg text-muted">Skipped (no API key).</p>;
  return (
...
function RenderNewsBlock({ payload }) {
  if (!payload) return <p className="text-lg text-muted">No news payload</p>;
  const blocks = [
  ...
  return (
    <div className="space-y-4">
      {blocks.map(([label, items]) => (
        <div key={label}>
          <p className="text-lg font-semibold uppercase text-muted">{label}</p>
          <ul className="mt-1 max-h-40 space-y-1 overflow-y-auto text-xl">
...
function RenderDiscoveryTable({ data }) {
  const ranked = data?.payload?.ranked;
  if (!ranked?.length) return <p className="text-lg text-muted">No ranked rows</p>;
  return (
...
function RenderMentionChart({ chart }) {
  if (!chart?.length) return <p className="text-lg text-muted">No chart rows</p>;
  return (
    <div className="h-48 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chart}>
          <CartesianGrid strokeDasharray="3 3" stroke="#1e2636" />
          <XAxis dataKey="symbol" stroke="#8b9cb5" tick={{ fontSize: 13 }} />
          <YAxis stroke="#8b9cb5" tick={{ fontSize: 13 }} />
          <Tooltip
            contentStyle={{
              background: "#121722",
...
function RenderLeaf({ section }) {
  ...
  if (id === "dataFeeds") {
    return (
      <div className="text-xl">
        {data?.err && <p className="text-lg text-rose-200/90">{data.err}</p>}
  ...
  if (id === "sidebarSelectedSymbol") {
    ...
    return (
      <SubCard title={title}>
        <p className="font-mono text-xl text-slate-200">{data?.symbol || "—"}</p>
        {a?.analysis && (
          <dl className="mt-2 space-y-1 text-lg text-slate-400">
  ...
  if (id === "sidebarAiInsight") {
    return (
      <SubCard title={title}>
        <p className="text-lg font-semibold text-violet-300">Claude</p>
        <p className="mt-1 whitespace-pre-wrap text-xl text-slate-300">
          {data?.insight?.text || "—"}
        </p>
        <p className="mt-3 text-lg font-semibold text-sky-300">ChatGPT</p>
        <p className="mt-1 whitespace-pre-wrap text-xl text-slate-300">
  ...
  if (id === "sidebarAiTradeView") {
    ...
    if (data?.loading) return <SubCard title={title}><p className="text-lg text-muted">Loading…</p></SubCard>;
    if (!p) return <SubCard title={title}><p className="text-lg text-muted">No data</p></SubCard>;
    return (
      <SubCard title={title}>
        {p.ruleTargets && (
        ...
        {p.claude?.thesis && <p className="mt-2 text-xl">{p.claude.thesis}</p>}
        {p.openai?.thesis && <p className="mt-2 text-xl">{p.openai.thesis}</p>}
      </SubCard>
    );
  }
  if (id === "sidebarRelatedHeadlines") {
    return (
      <SubCard title={title}>
        <ul className="max-h-40 space-y-2 overflow-y-auto text-xl">
          {(data?.items || []).map((n) => (
            <li key={n.id || n.title}>
  ...
  return (
    <SubCard title={title}>
      <p className="text-lg text-muted">Unknown block</p>
    </SubCard>
  );
}
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 rounded-lg border border-surface-border px-2 py-1 text-xl text-muted hover:text-white"
        >
          Close
        </button>
        <h2 id="history-modal-title" className="text-3xl font-semibold text-slate-50">
          History
        </h2>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button
            ...
            className="rounded-lg border border-emerald-500/40 bg-emerald-950/40 px-3 py-1.5 text-xl font-medium text-emerald-100 hover:border-emerald-400/60"
          >
            Export JSON
          </button>
          <button
            ...
            className="rounded-lg border border-sky-500/40 bg-sky-950/30 px-3 py-1.5 text-xl font-medium text-sky-100 hover:border-sky-400/60"
          >
            Import JSON
          </button>
          <input
          ...
          />
          <span className="text-lg leading-snug text-slate-500">
            Export uses “Save as…” in Chrome/Edge; other browsers download a uniquely named file.
          </span>
        </div>
        {banner && (
          <p
            className={`mt-3 rounded-lg border px-3 py-2 text-lg ${
              banner.type === "err"
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <label className="flex items-center gap-2 text-xl text-slate-300">
            ...
            <input
              ...
              className="rounded-lg border border-surface-border bg-surface px-2 py-1.5 text-xl text-slate-200"
            />
          </label>
          <button
            type="button"
            onClick={() => setViewDate(today)}
            className="rounded-lg border border-surface-border px-3 py-1.5 text-xl text-slate-200 hover:border-accent/40"
          >
            Today
          </button>
          {dates.length > 0 && (
            <span className="text-lg text-muted">{dates.length} day(s) stored</span>
          )}
        </div>

        <div className="mt-6 max-h-[calc(100vh-12rem)] space-y-6 overflow-y-auto pr-1">
          {!snapshot && (
            <p className="text-xl text-muted">No snapshot for this date yet. Use the board and check back.</p>
          )}
        ...
        {snapshot?.savedAt && (
          <p className="mt-4 text-lg text-slate-500">Last saved this day: {new Date(snapshot.savedAt).toLocaleString()}</p>
        )}
      </div>
    </div>
  );
